...
Wcs3 = tf.Variable(tf.random_uniform((100,300),-1,1))
...
curr_loss, curr_Output = sess.run([lossCS,OutputCS], {X4 : np_OutputCo3, Y :batch_ys})   
...
curr_loss, curr_Output = sess.run([lossCS,OutputCS], {X4 : np_OutputCo3, Y :mnist.test.labels})
...
print("Accuracy: ",sess.run(accuracy, feed_dict={X4: np_OutputCo3 ,Y: mnist.test.labels}))
